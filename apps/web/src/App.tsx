...
function CompanyPage() {
  ...
  return (
    <div className="page company-page">
      ...
      {company.evidence?.filings.length ? <section className="section-block"><div className="section-heading"><div><p className="eyebrow">SEC evidence</p><h2>Recent material filings</h2></div><Badge className="data-badge">SEC EDGAR</Badge></div><div className="news-list">{company.evidence.filings.slice(0, 6).map((filing) => <a className="card news-item filing-item" href={filing.url} target="_blank" rel="noreferrer" key={filing.accessionNumber}><Badge>{filing.form}</Badge><div><h3>{filing.primaryDocument}</h3><p>Filed {filing.filedAt}{filing.reportDate ? ` · reporting period ${filing.reportDate}` : ""}</p><small>{filing.provenance.source} · {filing.provenance.status}</small></div><ArrowRight size={17} /></a>)}</div></section> : null}
      <Disclaimer />
      ...
      <div className="assistant-layout">
        <Card className="chat-card">
          <div className="chat-scroll" aria-live="polite">
            ...
            {messages.map((message, index) => <div key={`${message.role}-${index}`} className={cn("chat-message", `chat-${message.role}`)}>{message.role === "assistant" && <span className="ai-orb"><Sparkles size={17} /></span>}<div>{message.answer?.mode && <Badge className="answer-mode">{message.answer.mode === "ai" ? "Workers AI · validated" : "Deterministic fallback"}</Badge>}<p>{message.text}</p>{message.answer && <><div className="answer-highlights">{message.answer.highlights.map((item) => <div key={item.label}><span>{item.label}</span><strong>{item.value}</strong></div>)}</div><div className="follow-ups">{message.answer.followUps.map((item) => <button key={item} onClick={() => submit(item)}>{item}</button>)}</div></>}</div></div>)}
